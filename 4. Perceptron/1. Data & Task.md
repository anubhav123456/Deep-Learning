...
This preserves much more information than the MP Neuron.
...
* $x$ = original value
* $\text{min}$ = minimum value of that feature
* $\text{max}$ = maximum value of that feature
...
$$x' = \frac{\text{min} - \text{min}}{\text{max} - \text{min}} = 0$$

So, **Minimum becomes 0.**
...
$$x' = \frac{\text{max} - \text{min}}{\text{max} - \text{min}} = 1$$

So, **Maximum becomes 1.**

---

### Every other value

Any value between $\text{min}$ and $\text{max}$ automatically becomes a number between **0 and 1**.

Therefore, after standardization:
...
![screen size](../images/4_Perceptron/1_Data_and_Task/screen_size.jpg)
![screen size](../images/4_Perceptron/1_Data_and_Task/battery.jpg)
![screen size](../images/4_Perceptron/1_Data_and_Task/data.jpg)
...
# 8. Task Supported by Perceptron
